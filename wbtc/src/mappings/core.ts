...
export function handleSwap(event: Swap): void {
  ...
  swap.user = event.params.sender.toHexString();
  ...
  if (swaps.length === 0) {
    ...
    user.usdSwapped = user.usdSwapped.plus(amountUsd);
    user.save();
  }

  log.debug("getting token balances {} {}", [pair.token0, pair.token1]);

  if (Address.fromString(pair.token0) == Address.fromString(BASE_ADDRESS)) {
    log.debug("token 0 is base {}", [swap.id]);
    swap.balance0 = getTokenBalance(Address.fromString(pair.token0), event.params.to);
  }
  if (Address.fromString(pair.token1) == Address.fromString(BASE_ADDRESS)) {
    log.debug("token 1 is base {}", [swap.id]);
    swap.balance1 = getTokenBalance(Address.fromString(pair.token1), event.params.to);
  }

  // update the UserTokenData
  // only do WBTC
  // if (token0.symbol === "WBTC") {
  //   updateUserTokenData(pair.token0, event.params.sender, amountUsd, event);
  // } else if (token1.symbol === "WBTC") {
  //   updateUserTokenData(pair.token0, event.params.sender, amountUsd, event);
  // }
